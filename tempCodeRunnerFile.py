...
real_time = real_data['date']  # Colonne 'date'
    ...
    for beta in beta_values:
        for delta in delta_values:
            for gamma in gamma_values:
                ...
                time, lapin, renard = lotka_volterra(alpha, beta, delta, gamma, 0.001, 1000)
...
# Tracé des courbes des meilleures prédictions
plt.figure(figsize=(15, 6))
plt.plot(best_time, best_pred_lapin, "b-", label="Lapins (prédiction)")
plt.plot(best_time, best_pred_renard, "r-", label="Renards (prédiction)")
plt.scatter(real_time, real_lapins, color="blue", label="Lapins (réel)", alpha=0.5)
plt.scatter(real_time, real_renards, color="red", label="Renards (réel)", alpha=0.5)
